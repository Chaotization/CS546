...
function checkMatricesInput(...args){
    ...
    args.map(item =>{
        ...
        if(index >= args.length &&(!Array.isArray(item) || !Array.isArray(args[index]) ||
            ...
            throw "Input is not an array or array is empty";
        }
        if(index >= args.length && item[0].length !== args[index].length){
            throw "The determinant does not exist"
        }
        Object.values(item).every(value => {
            if (!value.every(innerItem => typeof innerItem === "number")|| value.length !== len){
                throw "Inner arrays must only have numbers as elements or inner array is of the same length";
            }
        })
    })
}
export let sortAndFilter = (array, sortBy1, sortBy2, filterBy, filterByTerm) => {
    ...
    let sortedArr = array.filter(item=> item[filterBy] === filterByTerm);
    return sortedArr.sort((a, b)=> {
        if(a[sortBy1[0]] === b[sortBy1[0]]){
            return sortBy2[1] === 'asc' ? a[sortBy2[0]].localeCompare(b[sortBy2[0]]) : b[sortBy2[0]].localeCompare(a[sortBy2[0]]);
        }else{
            return sortBy1[1] === 'asc' ? a[sortBy1[0]].localeCompare(b[sortBy1[0]]) : b[sortBy1[0]].localeCompare(a[sortBy1[0]]);
        }
    })
 }
